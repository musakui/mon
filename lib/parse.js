...
const singleConditionRegex = /^(.+?)(?:__([A-Za-z]{2,3}))?:(.+)$/
...
export const parseColOption = (qs) => {
	const [col, name, cast] = qs.split(_p.arrayDelimiter)
	if (!col) return null
 ...
 * @param {string} str
 */
export const parseSingleCondition = (str) => {
	if (!str) return null
	const m = singleConditionRegex.exec(str)
	if (!m) return null
...
export const parseConditionOption = (qs) => {
	const whc = qs.split(_p.conditionDelimiter).flatMap((s) => {
		const c = parseSingleCondition(s)
		return c ? [c] : []
	})
	if (!whc.length) return null
	return whc.length === 1 ? whc[0] : whc
}
...
export const parseJoinOption = (qs) => {
	const [join, name, col, table] = qs.split(_p.arrayDelimiter)
...
export function* findConditions(params, cols) {
	for (const [k, v] of params) {
		...
		if (cols?.has(k) || (!cols && k[0] !== '_')) {
			yield { sql: k, op: `${OP.Equals}`, params: v }
			continue
		}
	}
}
...
export function parseSelectOptions(params) {
	...
	const where = [...findConditions(params), ...(wWhere.length ? [wWhere] : [])]
