...
export const processOpParams = (op, params) => {
	if (unary.has(op)) return /** @type {Operation} */ ([op])
...
export const stringifyOpParams = (op, params) => {
	...
	if (!code) return ['', '']
	if (unary.has(op)) return [code, '_']
	if (isStr(params)) return [code, params]
	if (!params) return [code, '']

	const proc = stringifyParam.get(op)
	if (!proc) return [code, `${params[0] ?? ''}`]

	return [code, proc(params)]
}
