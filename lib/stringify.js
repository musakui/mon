...
export function* stringifyCondition(s) {
	if (!s) {
		...
		for (const cond of s) {
			...
			const [code, param] = stringifyOpParams(cond.op, cond.params)
			if (!code) continue
			yield pp(cond.op === OP.Equals ? cond.sql : `${cond.sql}__${code}`, param)
		}
	} else {
...
export function* stringifyHavingCondition(s) {
	if (!s) {
		...
		const cond = stringifySingleCondition(s)
		if (cond) yield pp(_p.havingKey, cond)
	}
}
 ...
 *
 * @param {import('./types').JoinType | Lowercase<import('./types').JoinType>} [jt]
 */
export const getJoinKey = (jt) => {
	switch (jt?.toUpperCase()) {
		case 'FULL':
			return _p.fullJoinKey
		case 'LEFT':
			return _p.leftJoinKey
		case 'RIGHT':
			return _p.rightJoinKey
		default:
			return _p.joinKey
	}
}
...
export function* stringifyJoins(joins) {
	for (const jn of joins ?? []) {
		const val = joinArray([jn.table, jn.col, jn.to, jn.other])
